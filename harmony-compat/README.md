...
这是 Idea Recorder 1.0.0 的独立 Android 兼容版，面向支持 Android 应用的鸿蒙设备。它位于主项目的 `harmony-compat` 目录，使用独立 applicationId `com.idearecorder.harmony`，可以和 Android 版同时安装，互不覆盖本地数据。

> 说明：当前工作区没有 DevEco Studio/HarmonyOS SDK，因此这里交付的是鸿蒙 Android 兼容层可安装的 APK，不是 HarmonyOS NEXT 原生 HAP。原版项目目录未被修改。

中国大陆 Android 单机版的语音想法整理工具。当前版本是按 UI 设计稿重做的原生 Android 应用，运行时不依赖 Google Play Services、Firebase 或 Google 登录。
...
产物由主项目根目录的构建脚本复制到 `dist/Idea-Recorder-Harmony-1.0.0-debug.apk` 和 `dist/Idea-Recorder-Harmony-1.0.0-release.apk`。
